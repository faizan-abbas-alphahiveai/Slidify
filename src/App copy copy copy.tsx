...
// Helper: parse markdown + inline [icon:X[:optionalColorClass]]
const parseTextWithMarkdown = (text: string, keyPrefix: string) => {
  ...
  while ((match = markdownRegex.exec(text)) !== null) {
    ...
    if (linkText && linkUrl) {
      ...
      elements.push(<strong key={`${keyPrefix}-bold-${match.index}`}>{boldText}</strong>);
    } else if (italicText) {
      elements.push(<em key={`${keyPrefix}-italic-${match.index}`}>{italicText}</em>);
    }
...
export default function App() {
  ...
  const [slideshowToDelete, setSlideshowToDelete] = useState<{ id: string; name: string } | null>(
    null
  );
  const [isLoading, setIsLoading] = useState(false);
  const [shareUrl, setShareUrl] = useState("");

  const [imageSectionSubscriptionMessage, setImageSectionSubscriptionMessage] =
    useState<string | null>(null);
  const [musicSectionSubscriptionMessage, setMusicSectionSubscriptionMessage] =
    useState<string | null>(null);
  ...
  const [editingSubscriptionMessageId, setEditingSubscriptionMessageId] = useState<string | null>(
    null
  );

  // Dark mode to pick Lottie
  ...
  // Default name/message for new users (DB-driven)
  useEffect(() => {
  ...
  // Fetch user data + subscription messages
  useEffect(() => {
    if (user) {
      fetchUserSlideshows();
  ...
  const loadSubscriptionMessages = async () => {
    ...
    const imageMessage = await getSubscriptionMessage(subscriptionType, "images_section");
    setImageSectionSubscriptionMessage(imageMessage || null);

    const musicMessage = await getSubscriptionMessage(subscriptionType, "music_section");
    setMusicSectionSubscriptionMessage(musicMessage || null);
  };

  // Random tagline (DB-driven)
  useEffect(() => {
  ...
  const fetchUserSlideshows = async () => {
    ...
    if (!error) setUserSlideshows(data || []);
  };
  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setImages((prev) => {
      ...
      const availableSlots = isPremium ? Math.max(0, 100 - currentCount) : Math.max(0, 10 - currentCount);
      const filesToAdd = files.slice(0, availableSlots);
      if (files.length > availableSlots && availableSlots === 0) setShowUpgradeModal(true);
  ...
  const handleDrop = (event: React.DragEvent<HTMLDivElement>) => {
    ...
    setImages((prev) => {
      ...
      if (imageFiles.length > availableSlots && availableSlots === 0) setShowUpgradeModal(true);
      return [...prev, ...filesToAdd];
    });
  };
  ...
  const createShareableLink = async () => {
    ...
    if ((!isPremium && images.length > 10) || (isPremium && images.length > 100)) {
  ...
  // Open shared slideshow if `?view=` is present
  useEffect(() => {
    const urlParams = new URLSearchParams(window.location.search);
    const viewId = urlParams.get("view");
    if (viewId) fetchSharedSlideshow(viewId);
  }, []);
  ...
  return (
    <>
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-cyan-100 dark:from-gray-900 dark:to-gray-800 transition-colors">
            ...
            {images.length === 0 ? (
              <>
                ...
                >
                  ...
                  {!isPremium && (
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">(Max 10 for free users)</p>
                  )}
                  <input
                    type="file"
                    multiple
                ...
                {imageSectionSubscriptionMessage && (
            ...
            {!isPremium && (
              <div className="mt-4 bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-blue-900/20 dark:to-cyan-900/20 border border-blue-200 dark:border-blue-700 rounded-lg p-4 transition-colors">
                <div className="flex items-start gap-3">
                  <Crown className="text-blue-600 dark:text-blue-400 flex-shrink-0 mt-0.5" size={20} />
                  <div className="flex-1">
                    {imageSectionSubscriptionMessage && (
                      <p className="text-gray-800 dark:text-white text-sm mb-3">
                        {parseFormattedMessage(imageSectionSubscriptionMessage)}
                      </p>
                    )}
                    <button
        ...
        {showSuccessModal && (
          <SuccessModal isOpen={showSuccessModal} onClose={() => setShowSuccessModal(false)} slideshowName={slideshowName} shareableLink={shareUrl} />
        )}
      ...
      {showCollaborativeUploadModal && (
        <CollaborativeUploadModal
          ...
          onImagesAdded={(newImages) => setImages((prev) => [...prev, ...newImages])}
        />
      )}
